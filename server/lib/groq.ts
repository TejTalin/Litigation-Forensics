...
Every flag's reasoning must be concrete, not generic. The "explanation" must name the exact phrase and the specific legal consequence it risks -- never vague caution like "this could be risky." The "legal_basis" must cite the actual legal doctrine, section, or principle that makes it risky (e.g. "Sections 18-19, Limitation Act 1963" for a limitation-extension flag; "admission under law of evidence" for an admission flag; "without prejudice privilege doctrine" for a privilege-break flag; "waiver by conduct" for a waiver flag) -- not a made-up or vague citation. If uncertain of the precise section, state the general legal principle rather than inventing a citation. Never cite a specific named judicial precedent (e.g. "X v. Y") in this field unless you are certain it is a real, correctly-described, binding Indian authority directly on point -- when in doubt, cite only the statute, section, or doctrine by name, with no case citation at all. An invented or wrong-jurisdiction case name is a more serious error than a generic doctrinal citation.
